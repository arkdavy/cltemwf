...
# the whole config can be provided as a string of dictionary, or via a path to a new config.
...
# its elements can be updated via extra config, which must not change the default config structure, 
# i.e., it should keep the depth and the number of fields unchanged
...
if $bashiface; then
  ...
  # there is a helper function, which describes flags recognised by cltemwf
  cltemwf_batch --help

  # call to cltemwf. The extra configuration flag takes a dictionary here, which will be merge onto the default configuration.
  cltemwf_batch --cif Si.cif --config_extra ${config_extra} --files-to-copy '*.tif' --outdir 'outdir' -d gpu -s 2,2,2 -z 0,0,1

  # default config is provided by a file on the $config_path
  #cltemwf_batch --cif Si.cif --config=$config_path --config_extra ${config_extra} --files-to-copy '*.tif' -d gpu -s 2,2,2 -z 0,0,1

  # or we can set config directly from the dictionary, which makes config_extra practically unnecessary
  #cltemwf _batch--cif Si.cif --config ${config} -d gpu -s 2,2,2 -z 0,0,1
